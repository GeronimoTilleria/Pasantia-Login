...
6. Conectar el Cluster
	...
	Haz clic en "Connect" en la página del clúster. Luego, en la sección "Connect to your application", selecciona "Drivers". Aquí encontrarás la cadena de conexión para conectar tu backend (server) con la base de datos. Copia esa cadena y agrégala al archivo ".env" (en la carpeta server) de tu proyecto. La cadena de conexion de ir procedido por el nombre de la variable "MONGO_URL" seguido de "=" (la cadena de conexion debe ir dentro de comillas) quedando como algo así:
